/******************************************************************
PeerTree - Payment Object peerPayObj

2023-0123 - Taken from peerMemoryObj.js to be modified into the peerPayObj
*/
...
class peerPayToken{
   ...
   openWallet(){
      var keypair = null;
      try {keypair =  fs.readFileSync('keys/peerPayToken.key');}
	  ...
	  this.memCipher    = j.memCipher;
        ...
        fs.writeFile('keys/peerPayToken.key', wallet, function (err) {
          if (err) throw err;
...
class peerPayCellReceptor{
  constructor(peerTree){
    ...
    this.memToken = new peerPayToken();
  ...
  procQryResult(j){
    ...
    j.result.forEach((rec) =>{
      SQL = "insert into peerPay.qryLedgerResults ";
  ...
  getSearchResults(j){
    return new Promise( async(resolve,reject)=>{
      ...
      SQL += "from peerPay.qryLedgerResults where psrchHash = '"+skey+"' group by psrchMemoryID, psrchScore order by psrchScore desc";
      con.query(SQL, (err, result, fields)=> {
        if (err) {
 	  console.log(err);
  ...
  prepMemoryReq(j,res){
    ...
    var SQL = "SELECT pcelAddress FROM peerPay.payCells ";
...
class peerPayObj {
  ...
  updatePMemcellDB(j){
    ...
    var SQL = "SELECT count(*)nRec FROM peerPay.payCells where pcelAddress = '"+j.remIp+"'";
    con.query(SQL,(err, result, fields)=> {
      if (err) console.log(err);
      else {
        if (result[0].nRec == 0){
          SQL = "insert into peerPay.payCells (pcelAddress,pcelLastStatus,pcelLastMsg)";
          ...
          SQL = "update peerPay.payCells set pcelLastStatus = 'online',pcelLastMsg = now() ";
  ...
  doNodesDBMaint(){
    ...
    this.net.nodes.forEach((node) => {
      var SQL = "SELECT count(*)nRec FROM peerPay.payCells where pcelAddress = '"+node.ip+"'";
  ...
  resetDb(){
    return new Promise( (resolve,reject)=>{
      ...
      SQL += "truncate table peerPay.pLedger; ";
      con.query(SQL, async (err, result, fields)=>{
  ...
  doBestMatchQry(j,ip){
     ...
     var SQLr = "select pmcMownerID,pmcMemObjID,pmcMemObjNWords,count(*)nMatches,count(*)/pmcMemObjNWords score from peerPay.pLedger ";
  ...
  storeMemory(j,res){
    ...
    var SQLr = "insert into peerPay.pLedger (pmcMownerID,pmcMemObjID,pmcMemObjType,pmcMemObjNWords,pmcMemWord,pmcWordSequence) ";
    var SQL = "";
    var n = 1;
...
module.exports.peerPayObj = peerPayObj;
module.exports.peerPayCellReceptor = peerPayCellReceptor;
